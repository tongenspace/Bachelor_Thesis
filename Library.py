...
def Create_Hamiltonian(Model, S1, Interactions, Fields,d):
    ...
    for k in range(len(Model[1])):
      H += 1/2*np.einsum("ij,kl->ikjl",Fields[Model[1][k]]*S1[Model[1][k]],np.diag(np.ones(2)))    #on-site
      H += 1/2*np.einsum("ij,kl->ikjl",np.diag(np.ones(2)),Fields[Model[1][k]]*S1[Model[1][k]])
    return H

def Create_HamiltonianTEBD(Model, S1, Interactions, Fields,d):
    H = np.zeros((d,d,d,d)).astype(np.complex128)

    for j in range(len(Model[0])): #two-site
      H += np.einsum("ij,kl->ijkl",Interactions[Model[0][j]]*S1[Model[0][j]],S1[Model[0][j]])

    for k in range(len(Model[1])):
      H += np.einsum("ij,kl->ijkl",Fields[Model[1][k]]*S1[Model[1][k]],np.diag(np.ones(2)))    #on-site
      H += np.einsum("ij,kl->ijkl",np.diag(np.ones(2)),Fields[Model[1][k]]*S1[Model[1][k]])
    return H

def Create_initial_Hamiltonian(Model, S1, Interactions, Fields,d):
    H = np.zeros((d,d,d,d)).astype(np.complex128)
...
def Create_kraen_Hamiltonian(Model, S1, Interactions, Fields,d):
    H = np.zeros((d,d,d,d)).astype(np.complex128)
...
def Convert_to_A_and_B(Gamma, v, L):
  A,B = [],[]
  B.append(np.array([1.]))
  for j in range(L):
...
def Convert_to_A_and_B_mixer(Gamma, v, L):
  A,B = [],[]
  for j in range(L):
    A.append(np.einsum("a,aib->aib",v[j],Gamma[j]))
    B.append(np.einsum("aib,b->aib",Gamma[j],v[j+1])) 
...
def e_n(lambda_cut):
  return np.sum(np.square(lambda_cut))

def Truncation_error(en):
  return 1 - np.prod(1-2*en) 
